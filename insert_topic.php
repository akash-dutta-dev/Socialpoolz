...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = 'assets/images/topics_pic/'.$profile_id.'_temp.jpeg';
...
***********************************************************/	
	//Get Name | Size | Temp Location	
		$topic_name = $_POST['insert_topic_text'];
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/socialpoolz/assets/images/topics_pic';
	//Set File name	
	...
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$imgSrc= "assets/images/topics_pic/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];	
	//the image src
		$src = 'assets/images/topics_pic/'.$_POST['src'];	
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "assets/images/topics_pic/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="assets/images/topics_pic/".$finalname."n.jpeg";

		$insert_pic_query = mysqli_query($con, "INSERT INTO topic VALUES('','$topic_name','$result_path','0','0')");
		echo $_POST['x'];
...
?>
	...
	<div id="formExample">
	    ...
	    <form action="insert_topic.php" method="post"  enctype="multipart/form-data">
			...
			<input type="text" name="insert_topic_text" placeholder="Topic Name">
    ...
    if($imgSrc){ //if an image has been uploaded display cropping area?>
	    ...
	    <div id="CroppingContainer" style="width:800px; max-height:600px; background-color:#FFF; margin-left: -200px; position:relative; overflow:hidden; border:2px #666 solid; z-index:2001; padding-bottom:0px;    left: 500px;">  
	        ...
	        <div id="CropImageForm" style="width:100px; height:30px; float:left; margin:10px 0px 0px 40px;" >  
	            <form action="insert_topic.php" method="post" onsubmit="return checkCoords();">
	        ...
	        <div id="CropImageForm2" style="width:100px; height:30px; float:left; margin:10px 0px 0px 40px;" >  
	            <form action="insert_topic.php" method="post" onsubmit="return cancelCrop();">
